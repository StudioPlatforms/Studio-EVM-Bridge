// This script adds USDC as a new bridgeable token to the existing bridge
const hre = require("hardhat");
const fs = require('fs');
const path = require('path');
const { ethers } = hre;

// USDC token addresses - replace these with the actual USDC addresses on each chain
const BSC_USDC_ADDRESS = "0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d"; // USDC on BSC
const STUDIO_USDC_ADDRESS = "0xc6141A3680c0712198a122142047F90E2a2D73f8"; // Replace with actual USDC address on Studio

// Create a unique resource ID for USDC
// Resource IDs are 32 bytes, where the last byte is a chain identifier
// We'll use a similar pattern to the USDT resource ID but make it unique for USDC
const USDC_RESOURCE_ID = "0x9c1a1d9c2b109e527c9134b25b1a1833b16b6594f92daa9f6d9b7a6024bce9d0";
...
async function main() {
  try {
    ...
    const usdcAddress = network.name === 'bsc' ? BSC_USDC_ADDRESS : STUDIO_USDC_ADDRESS;
    console.log(`USDC address: ${usdcAddress}`);
    
    // Step 1: Set resource in the ERC20HandlerFixed contract
    console.log(`\nStep 1: Setting resource in ERC20HandlerFixed...`);
    console.log(`Mapping USDC resource ID (${USDC_RESOURCE_ID}) to USDC token address (${usdcAddress})...`);
    
    const setResourceTx = await erc20Handler.setResource(USDC_RESOURCE_ID, usdcAddress);
    ...
    console.log(`Mapping USDC resource ID (${USDC_RESOURCE_ID}) to ERC20Handler (${erc20HandlerAddress})...`);
    ...
    const setBridgeResourceTx = await bridge.setResource(USDC_RESOURCE_ID, erc20HandlerAddress, txOptions);
    ...
    console.log(`Auto-detecting decimals for USDC...`);
    const autoDetectDecimalsTx = await erc20Handler.autoDetectDecimals(USDC_RESOURCE_ID, txOptions);
    ...
    const decimals = await erc20Handler.getTokenDecimals(USDC_RESOURCE_ID, deploymentData.chainId);
    console.log(`Detected decimals for USDC: ${decimals}`);
    
    // Step 4: Update deployment file
    console.log(`\nStep 4: Updating deployment file...`);
    
    // Add USDC to resources
    deploymentData.resources.usdc = {
      resourceID: USDC_RESOURCE_ID,
    ...
    console.log(`\nUSCD has been successfully added to the bridge on ${network.name}!`);
    console.log(`Resource ID: ${USDC_RESOURCE_ID}`);
    ...
    console.error("Error adding USDC to bridge:", error);
