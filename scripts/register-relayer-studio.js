...
async function main() {
  console.log(`Registering relayer ${RELAYER_ADDRESS} on Studio chain...`);

  // Load deployment info
  const studioDeploymentPath = path.resolve(__dirname, '../studio-deployment.json');
  let studioDeploymentData;
  
  try {
    studioDeploymentData = JSON.parse(fs.readFileSync(studioDeploymentPath, 'utf8'));
  } catch (error) {
    console.error(`Error loading deployment data: ${error.message}`);
    console.error(`Make sure ${studioDeploymentPath} exists and is valid JSON.`);
    process.exit(1);
  }
  ...
  if (isRelayer) {
    console.log(`${RELAYER_ADDRESS} is already registered as a relayer on the Studio bridge.`);
    return;
  }
  ...
  if (isRelayerAfter) {
    console.log(`✅ ${RELAYER_ADDRESS} has been successfully registered as a relayer on the Studio bridge.`);
  } else {
    console.error(`❌ Failed to register ${RELAYER_ADDRESS} as a relayer on the Studio bridge.`);
  }
}
