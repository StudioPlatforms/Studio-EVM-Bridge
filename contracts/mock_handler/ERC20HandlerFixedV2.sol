...
contract ERC20HandlerFixedV2 is IHandler, HandlerHelpers {
    ...
    // Studio chain ID
    uint256 private constant STUDIO_CHAIN_ID = 240241;
    
    // Deposit nonce counter
    uint64 private _depositNonce;
    
    // Bridge contract address (the real bridge)
    address public constant BRIDGE_ADDRESS = 0x7FD526dC3d193a3dA6C330956813A6B358BCA1Ff;
    ...
    /**
     * @dev Set the resource ID to token contract address mapping
     * @param resourceID Resource ID for the token
     * @param contractAddress Address of the token contract
     */
    function setResourceIDToTokenContractAddress(bytes32 resourceID, address contractAddress) external onlyBridgeAdmin {
    ...
    function deposit(
        ...
        // If we're the owner, we can simulate a deposit directly
        if (msg.sender == _bridgeAddress) {
            // This is a normal bridge deposit
            ...
            // This is a simulated deposit by the owner
            // We'll make it look like it came from the bridge contract
            ...
            bytes memory proposalData = abi.encodePacked(
                resourceID,
                abi.encode(amount, BRIDGE_ADDRESS, recipient)
            );
            ...
            // Emit the deposit event directly, but make it look like it came from the bridge
            emit Deposit(
                destinationChainID,
                resourceID,
                _depositNonce,
                BRIDGE_ADDRESS,  // Make it look like the bridge initiated this
                recipientBytes,
                amount,
                dataHash
            );
            
            // Return empty bytes since we've handled everything
            return "";
        }
    }
    ...
    // Event definition to match the Bridge contract
    event Deposit(
